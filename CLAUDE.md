...
**Provider selection is "exactly one claimant" routing, not first-match.** `Codegraph.query`/`.index` (`packages/service/src/index.ts`) ask every registered store/indexer `indexes(projectRoot)`/`canIndex(projectRoot)` concurrently; zero claimants and multiple claimants both throw (`CODEGRAPH_UNAVAILABLE` / `CODEGRAPH_CONFLICT`) instead of picking by registration order. Keep this order-independence when adding a new provider. The *query* side of the rule is ancestor-aware: when no store claims the requested root, `query`, `available`, and `resolveRoot` walk up the root's directory chain (capped at 12 probes, stopping at the filesystem root) and route to the nearest directory some store claims, re-anchoring the request's `path`/`pattern` filters to that index root so they still match — a subdirectory of an indexed project answers from the project's index, and the tool reports it as `resolution_note`. The walk throws on a conflicted candidate instead of skipping to a farther one, and `index` stays exact-root-only: building a nested subdirectory index is the caller's explicit choice, never something routing invents.
