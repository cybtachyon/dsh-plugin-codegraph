...
索引的是**项目根**,不是已被索引的项目里的子目录:查询侧本来就会用最近的已索引祖先来回答子目录的调用(见下),子目录里再建一份索引只会盖住外面那份。同样不要索引一个只是*装着*项目的目录:向上查找时,高一层还能答,但那层若还吞进别的无关树,就没有任何好处。

指向一个**已被索引项目的子目录**的调用不算"没索引":选择逻辑会从给定的根向上走到有存储索引的最近目录,用那份索引作答,并把调用的 `path`/`pattern` 过滤器相应地重新锚定,匹配关系不变。所以工作区是某个已索引仓库里 `web/core` 的会话,可以直接查那个仓库的图;回答里会带一条 `resolution_note`,说明答案来自哪个根,模型不会把索引里的路径误当成相对于它给的那个子目录的路径。只有从给定根一直到文件系统根都没有任何目录被索引时,才适用下面的"没索引"回答——而且那时错误信息指向的是应该建索引的那个项目根,不是刚试过的那个目录。
